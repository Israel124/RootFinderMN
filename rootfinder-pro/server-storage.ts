...
async function saveJsonArray<T>(filePath: string, items: T[]) {
...
function mapCalculationRow(row: any): StoredCalculation {
  return {
    id: row.id,
...
export async function initStorage(usersFile: string, historyFile: string) {
  ...
  try {
    ...
    const jsonUsers = await loadJsonArray<StoredUser>(usersFile);
    ...
    const jsonHistory = await loadJsonArray<StoredCalculation & { userId?: string }>(historyFile);
    for (const item of jsonHistory) {
      if (!item.id || !item.userId || !item.method || !item.functionF) continue;
...
export async function findUserByEmail(usersFile: string, email: string): Promise<StoredUser | null> {
  if (pool) {
...
export async function markUserVerified(usersFile: string, email: string): Promise<StoredUser | null> {
  if (pool) {
  ...
  if (pool) {
    const result = await pool.query(
      `UPDATE users
       ...
       RETURNING *`,
      [verificationCode, expiresAt, Date.now(), email],
    );
    return result.rows[0] ? mapUserRow(result.rows[0]) : null;
  }
...
export async function listHistory(historyFile: string, userId: string) {
  if (pool) {
    const result = await pool.query(
  ...
  const history = await loadJsonArray<StoredCalculation & { userId?: string }>(historyFile);
  return history.filter((item) => item.userId === userId).slice(0, 50);
}

export async function saveHistoryItem(historyFile: string, userId: string, item: StoredCalculation) {
  if (pool) {
    await pool.query(
      `INSERT INTO calculations
        (id, timestamp, method, function_f, function_g, root, error, iterations, converged, message, params, label, user_id)
...
export async function updateHistoryItem(historyFile: string, userId: string, id: string, item: StoredCalculation | null, label: string | null) {
  if (pool) {
    if (item) {
...
export async function deleteHistoryItem(historyFile: string, userId: string, id: string) {
  if (pool) {
...
export async function clearHistory(historyFile: string, userId: string) {
  if (pool) {
