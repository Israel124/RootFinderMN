...
async function startServer() {
    ...
    app.post("/api/history", authenticateToken, async (req, res) => {
        ...
        try {
            const history = await loadHistory();
            const filtered = history.filter(h => h.id !== item.id || h.userId !== req.user.id);
    ...
    app.patch("/api/history/:id", authenticateToken, async (req, res) => {
        ...
        try {
            const history = await loadHistory();
            const index = history.findIndex(h => h.id === safeId && h.userId === req.user.id);
            if (index !== -1) {
                if (Object.keys(otherData).length > 0) {
                    const item = sanitizeCalculationPayload({ ...otherData, label });
                    history[index] = { ...item, timestamp: Date.now(), userId: req.user.id };
                }
                else {
    ...
    app.delete("/api/history/:id", authenticateToken, async (req, res) => {
        ...
        try {
            const history = await loadHistory();
            const filtered = history.filter(h => !(h.id === safeId && h.userId === req.user.id));
    ...
    app.delete("/api/history", authenticateToken, async (req, res) => {
        try {
            const history = await loadHistory();
            const filtered = history.filter(h => h.userId !== req.user.id);
            await saveHistory(filtered);
            res.status(200).json({ success: true });
        }
        catch (err) {
