...
async function startServer() {
    ...
    app.post("/api/history", async (req, res) => {
        if (!pool)
            return res.status(503).json({ error: "Base de datos no configurada (DATABASE_URL faltante)" });
        let item;
        try {
        ...
        DO UPDATE SET
          ...
          label = EXCLUDED.label`, [
            ...
            res.status(500).json({ error: "Error al guardar el cálculo en la base de datos: " + err.message });
        }
    });
    app.patch("/api/history/:id", async (req, res) => {
        ...
        try {
            if (Object.keys(otherData).length > 0) {
                ...
                await pool.query(`UPDATE calculations SET 
